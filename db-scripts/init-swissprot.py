...
parser = argparse.ArgumentParser(
    description='Filter Uniprot\'s Swissprot XML files to bacterial sequences and create initial PSTC db.'
)
...
parser.add_argument('--fasta', action='store', help='Path to Swissprot fasta file.')
...
with sqlite3.connect(str(db_path), isolation_level='EXCLUSIVE') as conn:
    ...
    with xopen(str(xml_path), mode='rb') as fh_xml, fasta_path.open('wt') as fh_fasta, alive_bar(enrich_print=False) as bar:
        for event, elem in et.iterparse(fh_xml, events=('end',)):
            ...
            if elem.tag.endswith('entry'):
                ...
                if not accession in seen_accessions:
                    ...
                    # Store in SQLite
                    conn.execute(
                        "INSERT INTO swissprot (id, product) VALUES (?, ?)",
                        (accession, product)
                    )
                    db_updates += 1
                    pstc_seqs += 1
    ...
    print(f"✅ Stored {pstc_seqs:,} Swissprot entries to {fasta_path}")

log_pstc.debug('summary: # Swissprot=%i', pstc_seqs)
